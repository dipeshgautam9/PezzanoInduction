...
//
// Also still supports legacy GEMINI_API_KEY (AIzaSy... or AQ.) as fallback.
...
// We sign a JWT with the private key and exchange it for a short-lived Bearer token.
// This token is cached for 50 minutes (tokens last 60 min).
...
export default async function handler(req, res) {
  ...
  if (identifyMode) {
    ...
    if (categories.length > 1) {
      ...
      } catch (e) { console.warn('identify step1 failed:', e?.message); }
    }
    ...
    try {
      ...
      if (parsed) return res.status(200).json({ productId: parsed.productId || null, productName: parsed.productName || null, confidence: typeof parsed.confidence === 'number' ? Math.max(0, Math.min(1, parsed.confidence)) : 0 });
      const idM = raw.match(/"productId"\s*:\s*"([^"]+)"/);
      const nmM = raw.match(/"productName"\s*:\s*"([^"]+)"/);
      const cfM = raw.match(/"confidence"\s*:\s*([\d.]+)/);
      return res.status(200).json({ productId: idM?.[1] || null, productName: nmM?.[1] || null, confidence: cfM ? parseFloat(cfM[1]) : 0 });
    } catch (err) {
  ...
  try {
    ...
    if (!parsed) {
      ...
      if (vm) return res.status(200).json({ verdict: vm[1].toLowerCase(), confidence: cm ? parseFloat(cm[1]) : null, similarity: sm ? parseInt(sm[1]) : null, explanation: em?.[1] || '', breakdown: [] });
